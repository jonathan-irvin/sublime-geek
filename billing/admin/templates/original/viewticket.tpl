...
{if $replyingadmin}<div class="errorbox">{$replyingadmin.name} viewed this ticket and started making a reply @ {$replyingadmin.time}</div>{/if}
</div>
...
<img src="images/spacer.gif" height="8" width="1" />
<br />
<div align="center"><input type="submit" value="Add Response" name="postreply" class="button" id="postreplybutton" /></div>
...
<tr><td rowspan="2" bgcolor="#F8F8F8" width="120" valign="top" align="left">{$note.admin}<br><a href="#" onClick="doDeleteNote('{$note.id}');return false"><img src="images/delete.gif" alt="Delete Ticket Note" border="0" align="absmiddle"></a></td><td bgcolor="#F8F8F8" align="left">{$note.date}</td></tr><tr><td bgcolor="#F8F8F8" align="left">{$note.message}</td></tr>
{/foreach}
</table>
{/if}
