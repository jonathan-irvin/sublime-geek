...
//Functions
request(string type){
    reqid = llHTTPRequest("http://www.sublimegeek.com/sg_admin/metacast/"
...
default
{
    ...
    timer(){
        if((stopwatch == 0)||(stopwatch == 30)){
            if(configactive){request("status");}            
        }
        
        //Reset the clock
        if(stopwatch <= 59){++stopwatch;}else{stopwatch = 0;}
    }
        ...
        if ((request_id == reqid) && power && configactive) {
            ...
            if(obj == "success"){
                if(cmd == "status"){
                    ...
                    if( (llList2String(commands,4) == "You are not broadcasting")   || 
                    ...
                    llSetText(
                        llGetObjectName()                             + "\n"   +
                    /*    "Username: "      + llList2String(commands,2) + "\n" + */
                    ...
                    if(llList2String(commands,3) == "up"){
                        llOwnerSay(obj+"! Your autoDJ is queued to be UP.  "+
                        "This will occur when no one else is connected to the stream.");
                        request("status");
                        color();
                    }
                    if(llList2String(commands,3) == "down"){                        
                        llOwnerSay(obj+"! Your autoDJ is queued to be DOWN.  "+
