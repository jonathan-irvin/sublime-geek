...
vector      pwron           = <0.48627, 0.74902, 1.0>;
vector      pwroff          = <0.33333, 0.33333, 0.33333>;

list        menu = 
            ["Stats"     ,"AutoDJ"   ,"Server",    
             "Display"   ,"Account"  ,"Reset",             
             "GetHUD"    ,"ReLoad"   ,"Instructions",
             "AdminPage" ,"Support"  ,"StreamPage"];
...
//Functions
request(string type){
    reqid = llHTTPRequest("http://sublimegeek.com/sg_admin/metacast/"
    +type+"/"+llEscapeURL(username),
        [0,"POST",1,"application/x-www-form-urlencoded"],"mccpass="+llEscapeURL(password));
}
command(string type, string status){
    reqid = llHTTPRequest("http://sublimegeek.com/sg_admin/metacast/"
    +type+"/"+llEscapeURL(username),
        [0,"POST",1,"application/x-www-form-urlencoded"],"mccpass="+llEscapeURL(password)+
        "&state="+llEscapeURL(status));
}
acct(string type, string attr){
    reqid = llHTTPRequest("http://sublimegeek.com/sg_admin/metacast/"
    +type+"/"+llEscapeURL(username),
        [0,"POST",1,"application/x-www-form-urlencoded"],"mccpass="+llEscapeURL(password)+
        "&attr="+llEscapeURL(attr));
}
acct_history(){
    reqid = llHTTPRequest("http://sublimegeek.com/sg_admin/admin/getlogs",
        [0,"POST",1,"application/x-www-form-urlencoded"],"mccpass="+llEscapeURL(password));
}
    
provision(string mail, string template, string duration){
    if(!DEBUG){
        reqid = llHTTPRequest("http://sublimegeek.com/sg_admin/metacast/newacct/"
        +llEscapeURL(username),
            [0,"POST",1,"application/x-www-form-urlencoded"],
            "mccpass="  +llEscapeURL(password)+
            "&email="   +llEscapeURL(mail)+
            "&template="+llEscapeURL(template)+
            "&duration="+llEscapeURL(duration));
    }else{
        //llOwnerSay("Setting up "+template+" for "+duration+" month");
        reqid = llHTTPRequest("http://sublimegeek.com/sg_admin/metacast/newacct/"
        +llEscapeURL(username),
            [0,"POST",1,"application/x-www-form-urlencoded"],
            "mccpass="  +llEscapeURL(password)+
...
init()
{
    ...
    //llSetLinkColor(LINK_SET,<.75,.75,0>,ALL_SIDES);
    llSetLinkColor(2,<.75,.75,0>,ALL_SIDES);
    llSetLinkColor(3,<.75,.75,0>,ALL_SIDES);
    llSetLinkColor(4,<.75,.75,0>,ALL_SIDES);    
    llSetLinkColor(5,<.75,.75,0>,ALL_SIDES);
    llSetLinkColor(6,<.75,.75,0>,ALL_SIDES);    
...
prov(integer amt){
    ...
    //if(DEBUG){llOwnerSay("Key: "+(string)id+" payed L$"+(string)amt);}
    
    //Select the server packages
    ...
    //if(DEBUG){llOwnerSay("Template: "+template);}        
}
...
default
{
    ...
    timer(){
        if((stopwatch == 0)||(stopwatch == 30)){
            if(configactive && power){request("status");}            
        }        
        ...
        if(llDetectedKey(0) == llGetOwner()){
            ...
            if(configactive && power){
                ...
                llDialog(llDetectedKey(0),
                    "MetaCast Cloud Controller\n"+
                    "Account Menu\n"+
                    "Did you need to setup a new account?",
                    ["NewAcct","Exit"],admchan);
            }else if(!power){
                llDialog(llDetectedKey(0),
                "MetaCast Cloud Controller\n"+
                "Make your selection...",
                ["Display On"],admchan);
            }else{llOwnerSay("Only the owner is allowed to activate the menu.");}
    ...
    listen(integer channel,string name,key id,string msg) {
        ...
        if(DEBUG){llOwnerSay((string)channel+": "+msg);}
        ...
        if((id == llGetOwner()) && (channel==admchan)){
            ...
            if(msg=="Display"){
                if(!power){
                    ...
                    "HUD Power Menu\n"+
                    "Make your selection...",
                    ["Display On","Back"],admchan);
                }else{
                    llDialog(id,
                    "MetaCast Cloud Controller\n"+
                    "HUD Power Menu\n"+
                    "Make your selection...",
                    ["Display Off","Back"],admchan);
                }
            }
            if(msg=="Account"){
                llDialog(id,
                ...
                "Account Menu\n"+
                "Make your selection...",
                ["AcctStatus","NewAcct","Back",
                 "Change","Renew","Usage","AcctLogs"],admchan);                
            }
            ...
            if(msg=="NewAcct"){
                ...
                "MetaCast Cloud Controller\n"+
                "New Account Menu\n"+
                "Please select the MetaCast Cloud Package...",
                [" "            ," "            ,"Back"         ,
                 "ShoutCast"    ,"IceCast"      ,"Reseller"]    ,admchan);
            }
            ...
            if(msg=="Change"){
                ...
                "Account Change Menu\n"+
                "Make your selection...",
                ["Upgrade","Downgrade","Back"],admchan);                
            }
            ...
            if(msg=="ShoutCast"){
                ...
                "Please select the MetaCast Cloud Package...",
                [" "            ," "            ,"Back"         ,
                 "Sh.Cast10"  ,"Sh.Cast25"  ,"Sh.Cast75"  ] ,admchan);
            }
            ...
            if(msg=="IceCast"){
                ...
                "Please select the MetaCast Cloud Package...",
                [" "            ," "            ,"Back"         ,                 
                 "IceCast10"    ,"IceCast25"    ,"IceCast75"    ] ,admchan);
            }
            ...
            if(msg=="Reseller"){
                ...
                "Please select the MetaCast Cloud Package...",
                [" "            ," "            ,"Back"         ,                 
                 "Reseller100"  ,"Reseller250"  ,"Reseller750"] ,admchan);
            }
            ...
            if ((msg=="Sh.Cast10")||(msg=="IceCast10")||(msg=="Sh.Cast25")||(msg=="IceCast25")||(msg=="Sh.Cast75")||(msg=="IceCast75")){
                ...
                if ((msg=="Sh.Cast10")||(msg=="IceCast10")) {
                    ...
                    quarterly   = (integer)((llList2Integer(prices,0)*.950)*  3);
                    biannually  = (integer)((llList2Integer(prices,0)*.925)*  6);
                    annually    = (integer)((llList2Integer(prices,0)*.800)* 12);                    
                ...
                if ((msg=="Sh.Cast25")||(msg=="IceCast25")) {
                    ...
                    quarterly   = (integer)((llList2Integer(prices,1)*.950)*  3);
                    biannually  = (integer)((llList2Integer(prices,1)*.925)*  6);
                    annually    = (integer)((llList2Integer(prices,1)*.800)* 12);     
                ...
                if ((msg=="Sh.Cast75")||(msg=="IceCast75")) {
                    ...
                    quarterly   = (integer)((llList2Integer(prices,2)*.950)*  3);
                    biannually  = (integer)((llList2Integer(prices,2)*.925)*  6);
                    annually    = (integer)((llList2Integer(prices,2)*.800)* 12);    
            ...
            if(msg=="Server"){                
                llDialog(id,
                    ...
                    "Server Menu\n"+
                    "Make your selection...",
                    ["Start","Stop","Back","Restart","Reload"],admchan);
            }
            ...
            if(msg=="AutoDJ"){                
                llDialog(id,
                    ...
                    "AutoDJ Menu\n"+
                    "Make your selection...",
                    ["StartAuto","KillAuto","Back","NextSong"],admchan);
            }
            ...
            if((msg=="Stats")||(msg=="Usage")){                
                llDialog(id,
                    ...
                    "Statistics Menu\n"+
                    "Make your selection...",
                    ["GetSongs","Bandwidth","Back","DiskSpace"],admchan);
            }
            ...
            if(msg=="StreamPage"){
                llLoadURL(llGetOwner(),"Your Personal Stream Page",
                "http://metacast.sublimegeek.com/start/"+username);
            }
            if(msg=="Display On"){
                power = TRUE;
                color();
    ...
    http_response(key request_id,integer status,list metadata,string body) {        
        ...
        if ((request_id == reqid) && power && configactive) {
            if(DEBUG){llOwnerSay("@"+(string)stopwatch+": "+body);}
            ...
            if(obj == "success"){
                if(cmd == "status"){
                    ...
                    string streamdata = 
                        "On-Air: "        + llList2String(commands,6) + "  "   +
                        "Listeners: "     + llList2String(commands,5) + "\n"   +
                        "Now Playing: "   + llList2String(commands,7) + "\n"   +                        
                        "Broadcasting on "+ llList2String(commands,9) + " @ "  +
                                            llList2String(commands,8) + "kbps\n" +
                        "Current Usage: " + float2string(mbph) + unit + " per hour";                    
                    
                    if( (llList2String(commands,4) == "Your broadcast is down")     || 
                        (llList2String(commands,4) == "")                           ||
                        (llList2String(commands,5) == "")                           ||
                        (llList2String(commands,6) == "")                           ||
                        (llList2String(commands,7) == "")                           ||
                        (llList2String(commands,8) == "")                           ||
                        (llList2String(commands,9) == "")                         )  {
                    ...
                    if(llList2String(commands,3) == "up"){
                        llOwnerSay("Success! "+"! Your autoDJ is queued to be UP.  "+
                        "This will occur when no one else is connected to the stream.");
                        request("status");
                        color();
                    }
                    if(llList2String(commands,3) == "down"){                        
                        llOwnerSay("Success! "+"! Your autoDJ is queued to be DOWN.  "+
                    ...
                    quarterly   = (integer)((llList2Integer(commands,3)*.950)*  3);
                    biannually  = (integer)((llList2Integer(commands,3)*.925)*  6);
                    annually    = (integer)((llList2Integer(commands,3)*.800)* 12);    
                ...
                "  \nPlease visit http://metacast.sublimegeek.com to login "+
                "and attempt your request via the online control panel.  "+
                "\nIf you need assistance, please create a ticket at http://support.sublimegeek.com");
                color();
                if(cmd == "provision"){llOwnerSay("In addition, no money was removed from your account.");}
                return;
    ...
    money(key id, integer amt){
        ...
        if(pvision){
            if(mail != ""){
                prov(amt);
                if(!DEBUG){llGiveMoney(gCreator,amt);}
            }else{llOwnerSay("Please update your email in the notecard");}
                pvision = FALSE;
        }
        
        if(upgrade){ 
            if(amt==1125){acct("upgrade","25GB");}        
            if(amt==3750){acct("upgrade","75GB");}        
            if(amt==4875){acct("upgrade","75GB");}
            if(!DEBUG){llGiveMoney(gCreator,amt);}
            upgrade = FALSE;
        }
        
        if(renew){ 
            if(amt==monthly)    {acct("renew","month");}        
            if(amt==quarterly)  {acct("renew","quarter");}        
            if(amt==biannually) {acct("renew","halfyear");}
            if(amt==annually)   {acct("renew","year");}
            if(!DEBUG){llGiveMoney(gCreator,amt);}
            renew = FALSE;
        }
        ...
        {            
            ...
            else
            {
                ...
                if((username == "")||(password == "")){
                    llOwnerSay("Hey! Open me and edit my configuration notecard inside"+
                    " and add your username and password.  I won't work without *both* pieces of info.");
