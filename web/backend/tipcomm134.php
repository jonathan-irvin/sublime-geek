...
$g_sql   = "SELECT * FROM `mtip_groups` WHERE `owner_api` = '$ok'";
...
if($g_num > 0){ //User has groups
  ...
  if($debug == TRUE){print("msg;GROUPS: TRUE\n");}
  ...
  if($ruser_num >= 1){//We found an existing user, do not create a new entry, just update it
    ...
    $payg_sql =  "INSERT INTO `mtip_pending_trans` 
        ...
        VALUES (NULL ,'$to','$usrchg',CURRENT_TIMESTAMP)";
    ...
    if($usrget >= 1){
      ...
      print("pay;After deductions... Net Pay: L$$usrget Awaiting Pmt: L$$usrchg Storing for future payouts
      If you need support, contact your club owner first, or visit http://www.sublimegeek.com/support/;$comprc");
    }else{
      print("pay;After deductions... Net Pay: L$$usrget Awaiting Pmt: L$$usrchg Storing for future payouts
      If you need support, contact your club owner first, or visit http://www.sublimegeek.com/support/;$comprc");
...
{ 
  ...
  $comsum = 0;    //Find the total amount of commission we are paying
  $totcom = 0;         //How much of the amt are we using for commission?
  $amtrem = $amt;         //What's left after the commission  
  ...
  if($nguser_num >= 1){//We found an existing user, do not create a new entry, just update it
    ...
    (`transid`,`owner_api`,`pmt_type`,`pmt_from`,`pmt_from_name`,`pmt_to`,`pmt_to_name`,`pmt_amt`,`timestamp`)
    VALUES  
    ('$transid','$ok','$ptype','$frm','$frm_name','$to','$to_name','$amt',CURRENT_TIMESTAMP)";
  ...
  //Message sent to usr in llOwnerSay
  print("pay;After deductions... Net Pay: L$$usrget Awaiting Pmt: L$$usrchg
  If you need support, contact your club owner first, or visit http://www.sublimegeek.com/support/;$comprc");
}
