...
//echo '<pre>';print_r($systems); echo '</pre>';exit;
    ...
    (integer) $fuel_uranium_size          = round($fuel_uranium           * 1);
    (integer) $fuel_oxygen_size           = round($fuel_oxygen            * 1);
    (float)   $fuel_mechanical_parts_size = round($fuel_mechanical_parts  * 1);
    (integer) $fuel_coolant_size          = round($fuel_coolant           * 2);
    (integer) $fuel_robotics_size         = round($fuel_robotics          * 2);
    (integer) $fuel_H_isotopes_size       = round($fuel_H_isotopes        * 0.15);
    (integer) $fuel_N_isotopes_size       = round($fuel_N_isotopes        * 0.15);
    (integer) $fuel_O_isotopes_size       = round($fuel_O_isotopes        * 0.15);
    (integer) $fuel_Hy_isotopes_size      = round($fuel_Hy_isotopes       * 0.15);
    (integer) $fuel_ozone_size            = round($fuel_ozone             * 0.4);
    (integer) $fuel_heavy_water_size      = round($fuel_heavy_water       * 0.4);
  ...
  $eve->SessionSetVar('errormsg', 'Buttweed doesn\'t have access!');
