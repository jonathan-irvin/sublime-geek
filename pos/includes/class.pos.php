 ...
 * @version    SVN: $Id: class.pos.php 242 2009-04-26 05:55:42Z stephenmg $
...
class POSMGMT
{
    ...
    function GetAllPos2($args)
    {
        ...
        $sql = "SELECT    ".TBL_PREFIX."tower_info.*,
                ...
                LEFT JOIN ".TBL_PREFIX."mapsolarsystems ms ON ".TBL_PREFIX."tower_info.systemID = ms.solarSystemID ".$orderby;
    ...
    {
        //Setup
        $required_isotope                    = $tower['isotopes'];
        $required_oxygen                     = $tower['oxygen'];
        $required_mechanical_parts           = $tower['mechanical_parts'];
        $required_coolant                    = $tower['coolant'];
        $required_robotics                   = $tower['robotics'];
        $required_uranium                    = $tower['uranium'];
        $required_ozone                      = $tower['ozone'];
        $required_heavy_water                = $tower['heavy_water'];
        $required_strontium                  = $tower['strontium'];
        $required_charters                   = $charters_needed?1:0;
        $race_isotope                        = $result['race_isotope'];
        $total_pg                            = $tower['pg'];
        $total_cpu                           = $tower['cpu'];
        //$tower['uptimecalc']                 = $posmgmt->uptimecalc($pos_id);
        $strontium_capacity                  = $tower['strontium_hangar'];
        ...
        $volume_per_cycle  = 0;
        $volume_per_cycle += $required_uranium;
        $volume_per_cycle += $required_oxygen;
        $volume_per_cycle += $required_mechanical_parts;
        $volume_per_cycle += ($required_coolant * 2);
        $volume_per_cycle += ($required_robotics * 2);
        $volume_per_cycle += ($required_isotope * 0.15);
        $volume_per_cycle += ceil((($current_pg / $total_pg) * $required_ozone) * 0.4);
        $volume_per_cycle += ceil((($current_cpu / $total_cpu) * $required_heavy_water) * 0.4);
    ...
    function GetAllTowers($args)
    {
        ...
        if (isset($args['pos_ids'])) {
            ...
            $sql = "SELECT * FROM `".TBL_PREFIX."tower_info` ".$where;
            ...
            $sql = "SELECT pos_id,
                    ...
                    AND    ".TBL_PREFIX."mapsolarsystems.regionID = '".$args['regionID']."'";
        } else {
...
//        } else {
     ...
     * @return
     */
    function selectstaticdb($sovereignty, $systemID, $allianceID=0)
    {

        $dbconn =& DBGetConn(true);

        // sql to grab the allianceid from the systemid sent, this turned out the be the best way of sending the data
        // This is kinda flawed, it assumes that the user is in the same alliance as the creator.  which shouldent be a problem, but that means multiple alliance tracking is no longer possible
        $sql = "SELECT * FROM `".TBL_PREFIX."system_status` WHERE solarSystemID ='".Eve::VarPrepForStore($systemID)."' LIMIT 1";
        $result = $dbconn->Execute($sql);
        ...
        $result->Close();
//$allianceID = Eve::SessionGetVar('allianceID');
//if ($systemID == '30002852') { echo '<pre>'.$allianceID;print_r($row);echo '</pre>';exit; }
        if ($row['allianceID'] == $allianceID)  { //new check if the data from the session data and the database match, changed to check provided option
            //Function to select the correct database depending on Sovereignty Level
            if ($sovereignty > 0) {
                if ($sovereignty>=4) {
                    $db = TBL_PREFIX.'sovereignty_constellation_static';
                } else {
                    $db = TBL_PREFIX.'sovereignty_static';
                }
            } else {
                $db = TBL_PREFIX.'tower_static';
            }
        } else {
            $db = TBL_PREFIX.'tower_static';
        }
    ...
    function uptimecalc($pos_id)
    {
        ...
        if ($row) {
            ...
            $sovereignty = $this->getSovStatus($systemID);
            //$pos_id = $row['pos_id'];
        ...
        $calc_isotope           = (floor($current_isotope          / $required_isotope))          - $hoursago;
        $calc_oxygen            = (floor($current_oxygen           / $required_oxygen))           - $hoursago;
        $calc_mechanical_parts  = (floor($current_mechanical_parts / $required_mechanical_parts)) - $hoursago;
        $calc_coolant           = (floor($current_coolant          / $required_coolant))          - $hoursago;
        $calc_robotics          = (floor($current_robotics         / $required_robotics))         - $hoursago;
        $calc_uranium           = (floor($current_uranium          / $required_uranium))          - $hoursago;
    ...
    /**
     * POSMGMT::uptimecalcOld()
     *
     * @param mixed $pos_id
     * @return
     */
    function uptimecalcOld($pos_id)
    {

        $dbconn =& DBGetConn(true);

        $sql = "SELECT * FROM pos_info WHERE pos_id = '" . Eve::VarPrepForStore($pos_id) . "'";
        $result = $dbconn->Execute($sql); //mysql_query($sql)         or die('Error retrieving from pos_info in function uptimecalc;' . mysql_error());
        if ($dbconn->ErrorNo() != 0) {
            Eve::SessionSetVar('errormsg', $dbconn->ErrorMsg() . $sql);
            return false;
        }
        ...
        $result->Close();
        if ($sovereignty == "1") {
            $db = "sovereignty_static";
        } else {
            $db = "pos_static";
        }
        $sql = "SELECT * FROM " . $db . " WHERE pos_size = '" . $pos_size . "'";
        //$result = mysql_query($sql) or die('Error retrieving from pos_static in function uptimecalc;' . mysql_error());
        $result = $dbconn->Execute($sql);
        if ($dbconn->ErrorNo() != 0) {
            Eve::SessionSetVar('errormsg', $dbconn->ErrorMsg() . $sql);
            return false;
        ...
        //}
        $result->Close();
        $sql = "SELECT * FROM pos_race_specific WHERE pos_race = '" . $pos_race . "'";
        $result = $dbconn->Execute($sql); //mysql_query($sql)         or die('Error retrieving from pos_info in function uptimecalc;' . mysql_error());
        if ($dbconn->ErrorNo() != 0) {
            Eve::SessionSetVar('errormsg', $dbconn->ErrorMsg() . $sql);
            return false;
        ...
        //if ($row = mysql_fetch_array($result)) {
            switch($pos_size) {
                case 1: $total_pg = $row['small_pg'];$total_cpu = $row['small_cpu'];break;
                case 2: $total_pg = $row['medium_pg'];$total_cpu = $row['medium_cpu'];break;
                case 3: $total_pg = $row['large_pg'];$total_cpu = $row['large_cpu'];break;
                case 4: $total_pg = $row['small_pg'];$total_cpu = $row['small_cpu'];break;
                case 5: $total_pg = $row['medium_pg'];$total_cpu = $row['medium_cpu'];break;
                case 6: $total_pg = $row['large_pg'];$total_cpu = $row['large_cpu'];break;
            }
        //}
        ...
        if ($calc_mechanical_parts <= "0") {
            $calc_mechanical_parts = "0";
        }
        if ($calc_coolant <= "0") {
            $calc_coolant = "0";
        }
...
//echo '<pre>';print_r($info);echo '</pre>';exit;
    ...
    function GetFuelBill($args = array())
    {
        ...
        foreach($alltowers as $tower) {
            ...
            $db = $this->selectstaticdb($sovereignty, $systemID, $allianceid);
            ...
            $sql = "SELECT * FROM ".TBL_PREFIX."pos_structures ps JOIN ".TBL_PREFIX."structure_static ss ON ps.type_id = ss.id WHERE ps.pos_id = '" . Eve::VarPrepForStore($pos_to_refuel) . "' AND ps.online=1";
            $result = mysql_query($sql) or die('Error retrieving from pos_structures in function uptimecalc;' . mysql_error());
            if (mysql_num_rows($result) != 0) {
                $current_pg  = "0";
                $current_cpu = "0";
        ...
        */
            ...
            //$bill[$pos_to_refuel]['owner'] = $row2['name'];
          ...
          /* --- Needed fuel calculation --- */
            ...
            $bill[$pos_to_refuel]['needed_uranium_size']          = $needed_uranium * 1;
            $bill[$pos_to_refuel]['needed_oxygen_size']           = $needed_oxygen  * 1;
            $bill[$pos_to_refuel]['needed_mechanical_parts_size'] = $needed_mechanical_parts * 1;
            $bill[$pos_to_refuel]['needed_coolant_size']          = $needed_coolant *  2;
            $bill[$pos_to_refuel]['needed_robotics_size']         = $needed_robotics * 2;
            $bill[$pos_to_refuel]['needed_isotopes_size']         = $needed_isotopes * 0.15;
            $bill[$pos_to_refuel]['needed_ozone_size']            = $needed_ozone *  0.4;
            $bill[$pos_to_refuel]['needed_heavy_water_size']      = $needed_heavy_water * 0.4;
            $bill[$pos_to_refuel]['needed_charter_size']          = $needed_charters * 0.1;
            if ($checkstront) {
                $bill[$pos_to_refuel]['needed_stront_size']       = $needed_stront * 3.0;
            }
        ...
        foreach ($xml->xpath('//row') as $row) {
            ...
            $sql = "UPDATE ".TBL_PREFIX."system_status
                    SET    allianceID               = '".Eve::VarPrepForStore($row['allianceID'])."',
                           constellationSovereignty = '".Eve::VarPrepForStore($row['constellationSovereignty'])."',
                           sovereigntyLevel         = '".Eve::VarPrepForStore($row['sovereigntyLevel'])."',
                           updateTime               = '".Eve::VarPrepForStore($time)."'
            ...
            if ($key['apitimer'] < ($time-3600)) { //21600 = 6 HOURS, The real time the API Caches the POS details information
                ...
                foreach ($xml->xpath('//row') as $pos) {
                    ...
                    $sql = "INSERT INTO ".TBL_PREFIX."update_log VALUES (NULL,
                                                                ...
                                                                '" . Eve::VarPrepForStore($updateTime) . "')";
                    $dbconn->Execute($sql);
                    if ($dbconn->ErrorNo() != 0) {
...
//echo '<pre>';print_r($rows); echo '</pre>';exit;
    ...
    function outpostUptimeCalc($outpostID)
    {
        ...
        $calc_helium_iso        = (floor($current_heilum_iso       / $total_req_helium_iso));
        $calc_hydrogen_iso      = (floor($current_hydrogen_iso     / $total_req_hydrogen_iso));
        $calc_oyxgen_iso        = (floor($current_oyxgen_iso       / $total_req_oxygen_iso));
        $calc_nitrogen_iso      = (floor($current_nitrogen_iso     / $total_req_nitrogen_iso));
        $calc_oxygen            = (floor($current_oxygen           / $total_req_oxygen));
        $calc_mechanical_parts  = (floor($current_mechanical_parts / $total_req_mechanical_parts));
        $calc_coolant           = (floor($current_coolant          / $total_req_coolant));
        $calc_robotics          = (floor($current_robotics         / $total_req_robotics));
        $calc_uranium           = (floor($current_uranium          / $total_req_uranium));
        $calc_strontium         =  floor($current_strontium        / $total_req_strontium);
