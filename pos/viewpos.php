 ...
 * @version    SVN: $Id: viewpos.php 243 2009-04-26 16:10:33Z stephenmg $
...
//echo '<pre>';print_r($tower);echo '</pre>';exit;
        ...
        /*
        $sql = "SELECT * FROM ".TBL_PREFIX."update_log WHERE type_id = '" . my_escape($pos_id) . "' AND type='1' ORDER BY id DESC";
        $result2 = mysql_query($sql) or die('Could not select from update_log; ' . mysql_error());
...
//echo '<pre>';print_r($miningmods);echo '</pre>';exit;
        ...
        /*
        $x = 0;
        //for ($linecount = 0; $linecount < count($starters); $linecount++) {
        foreach ($starters as $starter) {
            //$line = $posmgmt->GetConnectionLine(array('id' => $starter['mod_id'], 'type' => $starter['minetype'], 'pos_id' => $pos_id));

            $lines[$x][] = $miningmods[$starter['mod_id']]; //$line;

            if ($starter['link_to']) {
                //$line = $posmgmt->GetConnectionLine(array('id' => $line['link_to'], 'type' => $line['link_to_type'], 'pos_id' => $pos_id));
                ...
                //if ($line) {
                //    $lines[$x][] = $line;//echo '<pre>';print_r($lines);echo '</pre>';exit;
                //}
            } else {
                $x++;
                continue;
